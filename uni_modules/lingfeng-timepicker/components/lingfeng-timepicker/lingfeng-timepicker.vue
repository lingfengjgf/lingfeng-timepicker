      ...
      <view class="date-time-container" @click.stop="handleEvent">
        ...
        <view class="picker-tab" v-if="isShowRange">
          <view
            ...
            :style="[rangeBtnStyle, dateTab == 1 ? rangeBtnActiveStyle : '']"
          ...
          <view
            ...
            class="picker-tab-item"
            :style="[rangeBtnStyle, dateTab == 2 ? rangeBtnActiveStyle : '']"
            >{{ pickerData.endTime }}</view
          >
        </view>
        <picker-view
          class="picker-view"
          :indicator-style="popupIndicatorStyleString"
          :value="dateTime"
        ...
        >
          ...
          <picker-view-column data-id="quarter" v-if="isShowQuarter">
            <view class="item" v-for="(item, index) in quarters" :key="index">{{
              item
            }}</view>
          </picker-view-column>
        </picker-view>
...
export default {
  ...
  methods: {
    ...
    dateTabChange(i) {
      ...
      this.dateTab = i;
      this.datestring =
        i == 2 ? this.pickerData.endTime : this.pickerData.startTime;
      this.initDateTime();
      this.tempTime = new Date().getTime();
    },
        ...
        if (columns[0] != this.dateTime[0]) {
          ...
          if (this.dateTime[1] == 1) {
            if (this.dateTime[2] == this.days.length - 1) {
              if (
                getOneMonthDays(this.years[columns[0]], this.dateTime[1]) <
                getOneMonthDays(this.years[this.dateTime[0]], this.dateTime[1])
              ) {
        ...
        if (
          ...
          if (this.lastDateTime[0] != this.years[columns[0]]) {
            ...
            let index = this.months.findIndex(
              (val) => val == this.lastDateTime[1]
            );
            if (index < 0) {
              index =
                columns[0] == this.years.length - 1
            ...
            if (this.type != "year-month") {
              ...
              let d_index = this.days.findIndex(
                (val) => val == this.lastDateTime[2]
              );
              if (d_index < 0) {
                d_index =
                  this.lastDateTime[2] <= this.days[0]
                    ? 0
                    : this.days.length - 1;
              }
          ...
          if (
            ...
            this.lastDateTime[1] != this.months[columns[1]]
          ) {
            //月份变化后 查看日是否在范围 否则重新选择,this.days上面已更新
            let d_index = this.days.findIndex(
              (val) => val == this.lastDateTime[2]
            ...
            if (d_index < 0) {
              d_index =
                this.lastDateTime[2] <= this.days[0] ? 0 : this.days.length - 1;
            }
            columns.splice(2, 1, d_index);
          }
        } else if (this.type === "quarter") {
    ...
    formatDate() {
      ...
      if (
        ...
        formatDateArray = this.dateTime.map((item, index) => {
          let num = this[names[index] + "s"][item]
            ? this[names[index] + "s"][item]
            : this[names[index] + "s"][this[names[index] + "s"].length - 1];
          return addZero(num);
        });
        ...
        dateString =
          this.years[this.dateTime[0]] + " " + this.quarters[this.dateTime[1]];
      } else {
        ...
        dateString =
          formatDateArray.splice(0, 3).join("-") +
          " " +
          formatDateArray.join(":");
      }
    ...
    initDefaultDate() {
      ...
      if (this.type === "year") {
        ...
        let startTime = isOnlyTime(this.type)
          ? y + "/" + m + "/" + d + " " + this.pickerData.startTime
          : this.pickerData.startTime.replace(/-/g, "/");
        startTime = this.getMinDate(startTime);
        this.pickerData.startTime = isNaN(Date.parse(startTime))
          ? this.formatPickerData(new Date(), this.type)
          : this.formatPickerData(startTime, this.type);
        let endTime = isOnlyTime(this.type)
          ? y + "/" + m + "/" + d + " " + this.pickerData.endTime
          : this.pickerData.endTime.replace(/-/g, "/");
        startTime = this.getMinDate(endTime);
        this.pickerData.endTime = isNaN(Date.parse(endTime))
          ? this.formatPickerData(new Date(), this.type)
          : this.formatPickerData(endTime, this.type);
        this.datestring = this.pickerData.startTime;
      }
    },
    initDateTime() {
      ...
      if (this.datestring && this.datestring.length > 0) {
        if (this.type === "year") {
          ...
          value = isNaN(Date.parse(datestring))
    ...
    getDays(year, month) {
      ...
      if (
        year == this.minDate.year / 1 &&
        month + 1 == this.minDate.month / 1
      ) {
        start = this.minDate.date;
      }
      if (
        year == this.maxDate.year / 1 &&
        month + 1 == this.maxDate.month / 1
      ) {
    ...
    getMinDate(date) {
      let minArr = [
        this.minDate.year,
        addZero(this.minDate.month),
        addZero(this.minDate.date),
      ];
      let repDate = date.replace(/-/g, "/");
      let datetime = isNaN(Date.parse(repDate))
        ? new Date().getTime()
        : new Date(repDate).getTime();
      return datetime > new Date(minArr.join("/")).getTime()
        ? date
        : minArr.join("-");
    },
    getMAxDate(date) {
      let maxArr = [
        this.maxDate.year,
        addZero(this.maxDate.month),
        addZero(this.maxDate.date),
      ];
      let repDate = date.replace(/-/g, "/");
      let datetime = isNaN(Date.parse(repDate))
        ? new Date().getTime()
        : new Date(repDate).getTime();
      return datetime < new Date(maxArr.join("/")).getTime()
        ? date
        : maxArr.join("-");
    },
    //popup
  ...
  computed: {
    ...
    isShowMonth() {
      return !["year", "time", "hour-minute", "quarter", "time-range"].includes(
        this.type
      );
    },
    isShowDay() {
